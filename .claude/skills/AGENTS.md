...
- `Verify:` must assert **content**, not quantity or existence. The implementer is handed the command before it works (`implementer-prompt.md` step 4), so `grep -c … → 10` or `test -f …` is a bar it can clear without doing the task. A task whose only mechanical check is that weak does not qualify for `/implement`'s fast path — it goes to review, where a reviewer checks `Done When` independently
...
- STATE files track `**Current Wave**`, and every commit carries the STATE update for exactly the task IDs it contains — not batched to the wave boundary. `/implement` already commits per task's logical change, so the granularity was there; batching the STATE write was what made a run that died mid-wave redo finished work. STATE files written before this rule advance only at wave boundaries; the resume path detects and handles both
