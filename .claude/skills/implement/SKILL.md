...
3. **If the plan's `upstream-artifact:` frontmatter field names a path under `thoughts/shared/epics/`,** read that epic and carry out its `## Verification Plan (For Implementor)` section, then report the result. Skip this check if the plan's `upstream-artifact:` names a path under `thoughts/shared/features/`, a path under `thoughts/shared/changes/`, or the literal `none`. Only epics carry that section. The literal `none` reaches `/implement` in exactly two cases: QA-sourced plans where the QA report provides the target state, and legacy plans written before `/planner` gained its admission gate.
