...
- **Verify** — run the task's `Verify:` command. Does it produce the expected result?
...
`APPROVED` requires SPEC COMPLIANT, a passing `Verify:`, a holding `Done When`, and no Critical or Important issues.
